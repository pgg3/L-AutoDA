...
# from .draw_new import draw_proposals


class AutoDAttack(MinimizationAttack):
    distance = l2
...
def draw_proposals(
    ...
    # do the reshape work
    original_shape = originals_np.shape
    org_reshaped = originals_np.reshape(original_shape[0], -1)
    best_reshaped = best_advs_np.reshape(original_shape[0], -1)
    standard_noise_np = standard_noise_np.reshape(original_shape[0], -1)
    hyperparams_reshaped = hyperparams.reshape(original_shape[0], -1)

    noise_norm = np.linalg.norm(
        standard_noise_np, axis=1, keepdims=True
    )
    distance_vector = org_reshaped - best_reshaped
    d_parallel = np.square(noise_norm * hyperparams_reshaped) * distance_vector

    dot_value = np.matmul(distance_vector.reshape(distance_vector.shape[0], 1, -1),
                            standard_noise_np.reshape(standard_noise_np.shape[0], -1, 1)).reshape(-1, 1)
    distance_norm = np.linalg.norm(distance_vector, axis=1, keepdims=True)
    d_vertical = dot_value / distance_norm * distance_vector - distance_norm * standard_noise_np

    candidates_np = best_reshaped + d_parallel + d_vertical * hyperparams_reshaped
    return candidates_np.reshape(original_shape)